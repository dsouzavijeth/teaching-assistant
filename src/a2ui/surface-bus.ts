 ...
 * activity-message stream into the workspace canvas.
 *
 * Activity messages live inside CopilotChat's renderer scope; the workspace
 * lives outside it. The bus lets us forward ops between the two without
 * coupling React contexts.
 *
 * Per-thread (agentId) so multiple agents don't fight over the same canvas
 * state.
 */
